                    ...
                    <div class="px-3 py-4">
                        <div class="row align-items-center">
                            <div class="col-lg-4">
                                ...
                                <form action="http://localhost/eubazaars/seller/product/list" method="GET">
                                </form>
                            </div>
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex gap-3 justify-content-lg-end">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <a class="dropdown-item" href="{{route('admin.delivery-man.withdraw-list-export')}}">
                                ...
                                <select name="delivery_withdraw_status_filter" onchange="status_filter(this.value)" class="custom-select min-w-120 max-w-200">
                                    <option value="all" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'all'?'selected':''}}>{{translate('all')}}</option>
                                    <option value="approved" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'approved'?'selected':''}}>{{translate('approved')}}</option>
                                    <option value="denied" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'denied'?'selected':''}}>{{translate('denied')}}</option>
                                    <option value="pending" {{session()->has('delivery_withdraw_status_filter') && session('delivery_withdraw_status_filter') == 'pending'?'selected':''}}>{{translate('pending')}}</option>
                                </select>
